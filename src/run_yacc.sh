...
cc -o gocompiler.out y.tab.c lex.yy.c token_type.c error_handling.c abstract_syntax_tree.c linked_list.c

for test_case in $test_cases
do
	echo -e "Running test case ${test_case}"
	echo "----------------------------------------------------"
	./gocompiler.out -t < $test_case > "${test_case//${in_ext}/${out_ext}}"
